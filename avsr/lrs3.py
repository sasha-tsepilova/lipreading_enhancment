...
class LRS3(Dataset):
    def __init__(
        ...
        if subset == "train":
            self.args.root_dir = "../preprocessed_train"
            self.files, self.lengths = _load_list(self.args, "lrs3_train_transcript_lengths_seg4s.csv")
        if subset == "val":
            self.args.root_dir = "../preprocessed_test"
            self.files, self.lengths = _load_list(self.args, "lrs3_test_transcript_lengths_seg4s.csv")
        if subset == "test":
            self.args.root_dir = "../preprocessed_test"
            self.files, self.lengths = _load_list(self.args, "lrs3_test_transcript_lengths_seg4s.csv")
